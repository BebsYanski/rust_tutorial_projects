...
fn main() {
    ...
    let duration = match get_timer_input() {
        ...
        None => {
            println!(
                "Invalid input. Please enter numbers only (e.g., 0 1 30 for 0 hours, 1 minute and 30 seconds)."
            );
            return;
        }
    };
...
//  Add pause and resume features.
// Add alarm sound -> Audio notifications
